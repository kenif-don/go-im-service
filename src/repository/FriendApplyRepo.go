...
func (_self *FriendApplyRepo) QueryCount(obj *entity.FriendApply) (int, error) {
	var count int64
	tx := _self.Data.Db.Model(&entity.FriendApply{}).Where(obj).Count(&count)
	if tx.Error != nil {
		return 0, tx.Error
	}
...
func (_self *FriendApplyRepo) Save(obj *entity.FriendApply) error {
	tx := _self.Data.Db.Model(&entity.FriendApply{}).Where(obj.Id).Save(obj)
	if tx.Error != nil {
		return tx.Error
	}
	return nil
...
func (_self *FriendApplyRepo) Delete(obj *entity.FriendApply) error {
	tx := _self.Data.Db.Model(&entity.FriendApply{}).Where(obj).Delete(obj)
	if tx.Error != nil {
		return tx.Error
	}
	return nil
