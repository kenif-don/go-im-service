...
func (_self *ChatRepo) Save(obj *entity.Chat) error {
	tx := _self.Data.Db.Model(&entity.Chat{}).Where("type = ? and target_id = ? and user_id = ?", obj.Type, obj.TargetId, obj.UserId).Save(obj)
	if tx.Error != nil {
		return tx.Error
	}
	return nil
...
func (_self *ChatRepo) Delete(obj *entity.Chat) error {
	tx := _self.Data.Db.Model(&entity.Chat{}).Where(obj).Delete(obj)
	if tx.Error != nil {
		return tx.Error
	}
	return nil
