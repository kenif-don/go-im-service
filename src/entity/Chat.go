...
type Chat struct {
	...
	Type     string `json:"type" gorm:"index:idx_2,unique"`     // 聊天类型 friend, group
	TargetId uint64 `json:"targetId" gorm:"index:idx_2,unique"` // 聊天目标 用户ID或群ID
	UserId   uint64 `json:"userId" gorm:"index:idx_2,unique"`   // 当前聊天所有者 用户ID
